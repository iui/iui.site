<!DOCTYPE html>
<html>
<head>
	<meta http-equiv="Content-Type" content="text/html; charset=UTF-8" />
	<title>iUI - Sitemap</title>
	<meta name="description" content="iUI is a framework consisting of a JavaScript library, CSS, and images for developing advanced mobile webapps for smartphones & mobile devices.">
	<link href="medias/images/favicon.png" rel="icon" type="image/png" /> 
	<link href="medias/images/favicon.ico" rel="shortcut icon" />
	<link rel="image_src" type="image/jpeg" href="medias/images/thumb.jpg" />
	<meta http-equiv="X-UA-Compatible" content="IE=EmulateIE7" />

	<link rel='stylesheet' type='text/css' media="screen" href='medias/css/globals.css' />
	<link rel='stylesheet' type='text/css' media="screen" href='medias/css/sitemap.css' />
	<!--[if lt IE 7]><link rel="stylesheet" type="text/css" media="screen" href="medias/css/ie6.css"><![endif]-->

	<script type='text/javascript' src="medias/scripts/global.js"></script>
</head>

<body>

	<div id="main">

		<div id="menu">
			<ul>
				<li><span class="logo"></span></li>
				<li><a href="index.html" class="home"><span>Home</span></a></li>
				<li><a href="about.html" class="about"><span>About</span></a></li>
				<li><a href="demo.html" class="demo"><span>Demo</span></a></li>
				<li><a href="download.html" class="download"><span>Download</span></a></li>
				<li><a href="documentation.html" class="documentation"><span>Documentation</span></a></li>
				<li><a href="gallery.html" class="gallery"><span>Gallery</span></a></li>
				<li><a href="get-involved.html" class="involved"><span>Get Involved</span></a></li>
				<li><form method="GET" action="http://www.google.com/search" name="header_search">
					<a href="#" id="emptysearch" onclick="empty('search_q');hide(this.id)"></a>
					<input type="text" name="q" id="search_q" class="searchform" onFocus="checkHeaderSearch()" onKeyUp="checkHeaderSearch()" value="">
					<input type="hidden" name="sitesearch" value="iui-js.org">
					<input type="submit" value="search" class="hide">
				</form></li>
			</ul>

		</div>

		<hr>

		<div id="header">
			<h1>Sitemap</h1>
		</div>


		<div id="content">

			<div>
				<h2>Sitemap</h2>
			</div>

			<div>
				<div class="mid-col">
					<h3><a href="./">Home</a></h3>
					<ul>
						<li><a href="download.html">DOWNLOAD</a></li>
						<li><a href="demo.html">iUI video presentation</a></li>
						<li><a href="gallery.html">Powered by iUI</a></li>
						<li><a href="themes.html">Themes</a></li>
					</ul>
				</div>
	
				<div class="mid-col">
					<h3><a href="about.html">About iUI</a></h3>
					<ul>
						<li><a href="http://www.joehewitt.com/" target="_blank">Joe Hewitt</a></li>
						<li><a href="http://www.joehewitt.com/blog/introducing_iui.php" target="_blank">Joe Hewitt introduction blog post</a></li>
						<li><a href="http://code.google.com/p/iui/wiki/Roadmap" target="_blank">iUI Roadmap</a></li>
						<li><a href="http://en.wikipedia.org/wiki/IUI_(software)" target="_blank">http://en.wikipedia.org/wiki/IUI_(software)</a></li>
						<li><a href="documentation.html">Documentation</a></li>
						<li><a href="documentation.html">Extensions, sandbox & plugins</a></li>
						<li><a href="themes.html">Themes</a></li>
					</ul>
				</div>
			</div>
			
			<div>
				<div class="mid-col">
					<h3><a href="demo.html">iUI Video presentation demo</a></h3>
					<ul>
						<li><a href="documentation.html">Getting started documentation</a></li>
						<li><a href="themes.html">Themes</a></li>
						<li><a href="documentation.html">Advanced styling & themes</a></li>
					</ul>
				</div>
	
				<div class="mid-col">
					<h3><a href="download.html">Download</a></h3>
					<ul>
						<li><a href="documentation.html">Getting started documentation</a></li>
						<li><a href="http://code.google.com/p/iui/source/browse/" target="_blank">iUI source repository</a></li>
						<li><a href="documentation.html">Extensions, sandbox & plugins</a></li>
						<li><a href="themes.html">Themes</a></li>
					</ul>
				</div>
			</div>
			
			<div>
				<div class="mid-col">
					<h3><a href="documentation.html">Documentation (v0.50)</a></h3>
					<ul>
						<li><a href="documentation/0.50/getting-started.html">Getting started</a></li>
						<li><a href="faq.html">Frequently Asked Questions</a></li>
						<li><a href="documentation/0.50/iui-documentation.html">iUI code documentation</a></li>
						<li><a href="documentation/0.50/extensions.html">Extensions, sandbox & plugins</a></li>
						<li><a href="documentation/0.50/styles-themes.html">Advanced styling & themes</a></li>
					</ul>
				</div>
	
				<div class="mid-col">
					<h3><a href="download.html">Get Involved</a></h3>
					<ul>
						<li><a href="http://groups.google.com/group/iui-developers" target="_blank">Join iUI</a></li>
						<li><a href="http://code.google.com/p/iui/people/list" target="_blank">Meet the team</a></li>
					</ul>
				</div>
			</div>


		</div>

		<hr>

		<p id="footer">
			iUI &copy; 2007-2011 - iUI is an opensource framework released under MIT license - <a href="sitemap.html">sitemap</a> - <a href="donate.html">DONATE</a>
		</p>

	</div>
</body>
</html>